const cep = document.getElementById("cep") // pegando o elemento inteiro, transformadno a tag em html em objeto
...
const validarCep = async(cep)=>{ //validador de caracteres /  com função de SETA "Arrow Function"
    const cepValido = cep.length==8 &&/^^[0-9]+$/.test(cep)   //verdadeiro se o comprimento do cep for igual a 8 e se o caractres estão entre 0 a 9 e pega mais de um caracteres "+$"

    return cepValido // retorna o valor obtido
}
...
// Criando ações ao elementos para
pesquisar.addEventListener('click',pesquisarCep) // escutador de evento, quando acontecer o evento onclick, ira chamar uma função 
limpar.addEventListener('click',limnparDados) // escutador de evento
cep.addEventListener('focus',limparDados) // coloca foco quando o mouse esta cliclado no cep
